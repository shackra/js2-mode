...
(defvar js2-flow-primitive-types
  (mapcar 'symbol-name
          '(any bool boolean empty false mixed null
                number static string true typeof void))
...
;; Flow
(js2-msg "msg.flow.type.override"
         "cannot overwrite the primitive type: %s")

(js2-msg "msg.flow.only.import"
...
import ImportClause FromClause;"
  ...
  kind)      ; flow import type and typeof
...
(defun js2-flow-parse-import-kind ()
  "Match the import declaration kind for flow modules syntax.
...
"
  (let (kind curr-tt)
    (setq kind (cond
                ...
                (t nil)))
    (setq curr-tt (js2-current-token))
    (when kind
             ...
             (end-p (or (= next-tt js2-COMMA)
                        ...
                        (and (>= js2-ti-max-lookahead 3)
                             ...
                             (let ((next-tt2 (js2-next-token))
                                   match-as)
                               (setq match-as (and (= next-tt2 js2-NAME)
...
(defun js2-parse-import-clause (&optional import-kind)
...
(defun js2-parse-namespace-import (&optional import-kind)
...
(defun js2-flow-validate-primitive-type ()
  "Test export name was flow's primitive type.
like:
import type any from 'm';
or
import type foo as mixed from 'm';
"
  (let* ((name (js2-current-token-string))
         (pos (js2-current-token-beg))
         (len (string-width name)))
    (when (member name js2-flow-primitive-types)
      (js2-report-error "msg.flow.type.override" name pos len))))

(defun js2-parse-export-bindings (&optional import-p import-kind)
...
js2-LC. Return a lisp list of js2-export-binding-node"
  (let ((bindings (list)))
    (while
        (let ((binding (js2-maybe-parse-export-binding import-p import-kind)))
          (when binding
...
(defun js2-maybe-parse-export-binding (&optional import-p import-kind)
  ...
  (let ((kind (js2-flow-parse-import-kind))
    ...
    (when (and import-kind kind)
      ...
      (js2-report-error "msg.flow.only.import" nil
                        ...
                        (string-width (js2-token-string import-kind))))
    (setq only-kind (or (and import-kind (not kind))
                        (and (not import-kind) kind)))
...
;;;###autoload
(define-derived-mode js2-flow-mode js-mode "Javascript-IDE"
